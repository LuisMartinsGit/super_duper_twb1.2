    ...
    public static class BorderNodeBootstrap
    {
        ...
        public static int SpawnBorderNodes()
        {
            ...
            if (!MapMarkerRegistry.HasBorderMarkers)
            {
                ...
                // Still build the faction state. This return used to skip
                // EnsureBorderFactionState entirely, so on a markerless map the
                // BorderExtinctionState and NodeVictoryState singletons were
                // never created — and both systems RequireForUpdate them, so they
                // stayed PARKED FOR THE WHOLE MATCH. The curse could then never
                // respawn after extinction and well-domination victory was
                // silently unreachable, with nothing in the log to say so.
                EnsureBorderFactionState(em, 0);
                return 0;
            }
