// File: Assets/GameData/TechTree/Buildings/Runai/TradingPost/TradingPostSystem.cs
    ...
    public partial struct TradingPostSystem : ISystem
    {
        ...
        private void SpawnTraders(ref SystemState state, EntityManager em, float dt)
        {
            ...
            for (int i = 0; i < traderRequests.Length; i++)
            {
                ...
                // Through the factory (not Caravan.Create directly) so the
                // caravan gets the central stamps: UnitTypeId, DisplayName,
                // NetworkedEntity and the per-faction counter tags.
                Entity trader = TheWaningBorder.Entities.UnitFactory.Create(
                    em, "Runai_Caravan", req.SpawnPos, req.Faction);
