...
//   * SUSTAIN  — nothing to do here: live small nodes are folded into the CA
//                feeder set by VeilFieldSystem, so the patch feeds itself.
//   * STARVE   — a small node standing in suppressed ground (Hall hearth or
//                any player influence >= threshold) loses HP each tick;
//                warding a pocket kills it in ~SmallNodeHealth/StarveDps s.
//   * COLLAPSE — when a pocket's small node dies (combat or starvation; the
...
namespace TheWaningBorder.Systems.Border
{
    ...
    public partial class BlightPocketSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            // (VeilstoneMiningSystem pings + queues them; we own the rise).
            {
                ...
                if (regs.Length > 0 && em.HasBuffer<PendingCorruption>(regs[0]))
                {
                    ...
                    {
                        var smallNode = SmallNode.Create(em, due[i]);
            ...
            for (int i = 0; i < buffer.Length; i++)
            {
                ...
                bool alive = pocket.SmallNode != Entity.Null
                    && em.Exists(pocket.SmallNode)
                    && em.HasComponent<Health>(pocket.SmallNode)
                    && em.GetComponentData<Health>(pocket.SmallNode).Value > 0;
                ...
                if (alive)
                {
                    ...
                    // DestroyEntity here.
                    var pos = em.GetComponentData<LocalTransform>(pocket.SmallNode).Position;
                    if (SuppressedAt(pos))
                    {
                        var hp = em.GetComponentData<Health>(pocket.SmallNode);
                        hp.Value -= (int)math.ceil(SmallNodeStarveDps * TickInterval);
                        if (hp.Value < 0) hp.Value = 0;
                        em.SetComponentData(pocket.SmallNode, hp);
                    }
                    continue;
                }
        ...
        /// the handful of small nodes.</summary>
