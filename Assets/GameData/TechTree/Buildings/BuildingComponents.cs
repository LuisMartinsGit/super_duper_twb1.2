...
/// TempleChapelRing for the shared face-aligned ring geometry.
/// </summary>
...
public struct TempleOwner : IComponentData
{
    ...
    /// <summary>Slot index (0-5) in the parent temple's TempleChapelSlot buffer.</summary>
    public int SlotIndex;
}

/// <summary>
/// Shared geometry for the temple's chapel ring. The temple is a 7-sided
/// cathedral with its door on face 0 (+Z); chapel slot i (0-5) sits on face
/// i+1, so slots, site decals, and spawned chapels all agree on position.
/// </summary>
public static class TempleChapelRing
{
    ...
    public static Unity.Mathematics.float3 WorldOffset(int slotIndex)
    {
        // Face i+1 of the heptagon, measured as yaw from +Z (the door face).
        float theta = (slotIndex + 1) * (2f * Unity.Mathematics.math.PI / 7f);
        return new Unity.Mathematics.float3(
            Unity.Mathematics.math.sin(theta) * SlotRadius,
            0f,
            Unity.Mathematics.math.cos(theta) * SlotRadius);
    }
}
