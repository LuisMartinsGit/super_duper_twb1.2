// File: Assets/GameData/TechTree/Units/Alanthor/Ballista/CatapultVisual.cs
...
namespace TheWaningBorder.Presentation
{
    public class CatapultVisual : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            float cd = _em.GetComponentData<ArcherState>(e).CooldownTimer;
        ...
        private void SpawnShotFx(Entity e)
        {
            ...
            // Ballistic solve: the elevation that impacts the GROUND at the
            // target's location, from the actual muzzle position. For launch
            // speed v, gravity g, horizontal distance x, and height delta y
            // (target ground minus muzzle):
            //   tan(theta) = (v^2 - sqrt(v^4 - g*(g*x^2 + 2*y*v^2))) / (g*x)
            // (low branch). Negative discriminant = physically out of the
            // stone's reach — fall back to the maximum-range elevation.
            ...
            if (hasTargetPos)
            {
                ...
                if (x > 0.5f)
                {
                    ...
                    float theta = disc >= 0f
                        ? Mathf.Atan2(v2 - Mathf.Sqrt(disc), g * x)
                        : Mathf.Atan2(v2, g * x);
