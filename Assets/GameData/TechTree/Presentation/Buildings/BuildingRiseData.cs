...
namespace TheWaningBorder.Presentation
{
    public class BuildingRiseData : MonoBehaviour
    {
        ...
        // FULL local rest position, not just Y. The sink direction is derived
        // from world up per piece (see ApplyRise), so the offset can land on
        // any local axis and the restore has to put all three back.
        ...
        public void Init()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                _restLocal[i] = collected[i].localPosition;
        ...
        /// <summary>
        /// The LOCAL offset that sinks a piece by <paramref name="sinkDepth"/>
        /// world units straight down.
        /// <para>
        /// Uses InverseTransformVector (rotation AND scale), not
        /// InverseTransformDirection (rotation only). Imported FBX hierarchies
        /// carry a unit-conversion scale — measured at 100x on the Blender
        /// building exports — so treating a world-space sink depth as a local
        /// offset buried the pieces a hundred times too deep and they streaked
        /// in from off-screen. Hand-authored prefabs sit at scale 1, where this
        /// is identical to the old maths.
        /// </para>
        /// </summary>
        private static Vector3 LocalSink(Transform piece, float sinkDepth)
        {
            Vector3 world = Vector3.down * sinkDepth;
            var parent = piece.parent;
            return parent == null ? world : parent.InverseTransformVector(world);
        }
        ...
        public void ApplyRise(float ratio, float sinkDepth)
        {
            ...
            for (int i = 0; i < _pieces.Length; i++)
            {
                ...
                // Sink along WORLD down, expressed in the piece's parent space
                // — not along local -Y. Models exported from Blender carry the
                // Z-up -> Y-up conversion as a -90 deg X rotation on the root
                // node, so under that root a local +Y offset points along world
                // -Z: the pieces slid in horizontally instead of rising out of
                // the ground. Identical to the old maths whenever the parent is
                // unrotated, so hand-authored prefabs are unaffected.
                c.localPosition = _restLocal[i] + LocalSink(c, sinkDepth) * (1f - eased);
            }
        ...
        public bool NotifyConstructionComplete()
        {
            ...
            for (int i = 0; i < _pieces.Length; i++)
            {
                ...
                _pieces[i].localPosition = _restLocal[i];
            }
            return true;
        }
    }
