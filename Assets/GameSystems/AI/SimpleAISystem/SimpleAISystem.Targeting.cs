...
namespace TheWaningBorder.AI
{
    public partial class SimpleAISystem : SystemBase
    {
        ...
        static readonly ComponentType[] QT_VeilField =
        {
            ComponentType.ReadOnly<VeilField>(),
        };
        static CachedEntityQuery QC_VeilField;
        ...
        static readonly ComponentType[] QT_SmallNodeTagHealthLocalTransform =
        {
            ComponentType.ReadOnly<SmallNodeTag>(),
            ComponentType.ReadOnly<Health>(),
            ComponentType.ReadOnly<LocalTransform>(),
        };
        static CachedEntityQuery QC_SmallNodeTagHealthLocalTransform;
        ...
        {
            var q = QC_VeilField.Get(em, QT_VeilField);
        ...
        private static bool CurseBlocksCorridor(EntityManager em, float3 a, float3 b)
        {
            ...
            int samples = math.max(2, (int)(dist / Cfg.curseCorridorSampleStep));
            int crusted = 0;
            ...
            return crusted >= (samples + 1) * Cfg.curseCorridorHeavyFraction;
        }
            ...
            for (int i = 0; i < ents.Length; i++)
            {
                ...
                for (int sIdx = 0; sIdx < starts.Count && !atStart; sIdx++)
                {
                    ...
                    atStart = ddx * ddx + ddz * ddz
                              <= Cfg.startHallKnownRadius * Cfg.startHallKnownRadius;
                }
                if (!atStart) continue;
        ...
        {
            var q = AIQueryCache.TagFactionXf<TTag>(em);
            using var ents = q.ToEntityArray(Allocator.Temp);
            using var facs = q.ToComponentDataArray<FactionTag>(Allocator.Temp);
            using var xfs = q.ToComponentDataArray<LocalTransform>(Allocator.Temp);
