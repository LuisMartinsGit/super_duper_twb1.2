...
namespace TheWaningBorder.AI
{
    public partial class SimpleAISystem : SystemBase
    {
        ...
        static readonly ComponentType[] QT_BuildingTagFactionTagHealthLastAttackerEntityLocalTransform =
        {
        ...
        static CachedEntityQuery QC_BuildingTagFactionTagHealthLastAttackerEntityLocalTransform;
        ...
        static readonly ComponentType[] QT_CanBuildFactionTag =
        {
            ComponentType.ReadOnly<CanBuild>(),
            ComponentType.ReadOnly<FactionTag>(),
        };
        static CachedEntityQuery QC_CanBuildFactionTag;
            ...
            ref SimpleAIState aiState, AISettingsSO settings, AISettingsSO.PersonalityBlock personality)
        {
            ...
            //
            // SCOPED TO THE HOME AREA (2026-09-03). The scan used to cover
            // every building the faction owned, map-wide — so a razed border
            // outpost 400 u away disbanded every mission and recalled a
            // 138-unit army from the enemy capital, over and over (Blue
            // entered Defend 35 times in one logged match, flapping every
            // 40-70 s while holding a 135-vs-29 army lead it never used).
            // You do not recall the army for a hut: distant outposts defend
            // themselves with auto-acquire or are lost, which is correct RTS
            // play. The home economy stays fully covered.
            if (TryFindAttackedBuilding(em, faction, hallPos,
                    settings.defendRadius * 2.5f, out float3 attackedPos))
            {
                bool entering = aiState.Posture != AIPosture.Defend;
                aiState.Posture = AIPosture.Defend;
                if (entering)
        ...
        private static bool TryFindAttackedBuilding(EntityManager em, Faction faction,
            float3 hallPos, float maxRange, out float3 pos)
        {
            pos = default;
            float maxRangeSq = maxRange * maxRange;
            var bq = QC_BuildingTagFactionTagHealthLastAttackerEntityLocalTransform.Get(em, QT_BuildingTagFactionTagHealthLastAttackerEntityLocalTransform);
            ...
            for (int i = 0; i < ents.Length; i++)
            {
                ...
                // Home-area scope — see the call site.
                float hdx = xfs[i].Position.x - hallPos.x;
                float hdz = xfs[i].Position.z - hallPos.z;
                if (hdx * hdx + hdz * hdz > maxRangeSq) continue;
        ...
        private void DefendBase(EntityManager em, Faction faction, float3 hallPos, AISettingsSO settings)
        {
            ...
            var cq = QC_CanBuildFactionTag.Get(em, QT_CanBuildFactionTag);
            ...
            for (int i = 0; i < cEnts.Length; i++)
            {
                ...
                if (AICommon.IsCommittedWorker(em, cEnts[i])) continue; // building/repairing already
