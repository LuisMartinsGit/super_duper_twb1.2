...
namespace TheWaningBorder.AI
{
    public partial class SimpleAISystem : SystemBase
    {
        ...
        static readonly ComponentType[] QT_BuildingTagFactionTagResearchQueueItem =
        {
            ...
            AIDifficultyProfile profile, float now)
        {
            ...
            int scoutTarget = math.clamp(
                ...
                2, 6);
            // Count the QUEUE too: alive-only counting re-ordered a scout
            // every 2 s think tick until the first one spawned — 10-13
            // trained against a target of 2, draining the claim pot.
            if (CountScouts(em, faction)
                + AICommon.CountQueued(em, faction, "Scout") < scoutTarget)
            ...
            AIDifficultyProfile profile, float now)
        {
            ...
            _nextEmergency[key] = now + Cfg.emergencyRetrySeconds;
            ...
            if (res.Supplies + res.Iron < Cfg.emergencyBankFloor) return;

            if (CountFactionBuildings<WatchTowerTag>(em, faction) < Cfg.emergencyTowerCap
            ...
            if (CountFactionBuildings<SiegeYardTag>(em, faction) == 0)
            {
                if (TryBuildBuilding(em, faction, "Alanthor_SiegeYard"))
                {
                    ...
                    AILogger.Log(faction, "MILITARY", "siege program: Siege Yard started");
                         ...
                         && yard != null)
                {
                    AIPivotalReserve.Set(faction, "SiegeYard", AICommon.ToCost(yard.cost));
                }
                return;
            }
            AIPivotalReserve.Clear(faction, "SiegeYard");
            ...
            AIDifficultyProfile profile, float now)
        {
            ...
            // tilts the weights, this pipeline throttles by itself).
            {
                ...
                else if (ghTotal >= Cfg.hutPipelineFreeCount
        ...
        private static bool LadderHasAffordableStep(EntityManager em, Faction faction)
        {
            ...
            for (int i = 0; i < ladder.Length; i++)
            {
                ...
                if (FactionEconomy.CanAfford(em, faction, AICommon.ToCost(def.cost))) return true;
            }
            return false;
        }
        ...
        private static void TryTrainPivotalUnique(EntityManager em, Faction faction, string unitId)
        {
            ...
            {
                AIPivotalReserve.Set(faction, unitId, AICommon.ToCost(def.cost));
            }
