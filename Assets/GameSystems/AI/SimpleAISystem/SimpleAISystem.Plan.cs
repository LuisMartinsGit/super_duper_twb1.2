...
namespace TheWaningBorder.AI
{
    public partial class SimpleAISystem : SystemBase
    {
            ...
            bool emergency, out string reason)
        {
            ...
            // came down.
            //
            // UNLESS THE BOARD IS ALREADY WON (2026-09-03). Forcing Fortress
            // on every Defend flap raised the wave bar to 2.2x (need 11 idle)
            // for a faction holding a 135-vs-29 army lead — Blue flapped
            // MASSING->FORTIFYING every 40-70 s for eleven minutes and never
            // launched the push that would have ended the match. A dominated
            // opponent's raid is a nuisance the posture layer already answers
            // (the home recall still fires); it must not also cancel the war.
            if (emergency)
            {
                bool dominant = b.MyArmy >= 20
                    && b.MyArmy >= b.TopEnemyArmy * 2;
                if (!dominant)
                {
