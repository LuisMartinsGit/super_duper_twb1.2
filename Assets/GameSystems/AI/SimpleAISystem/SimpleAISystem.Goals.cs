...
namespace TheWaningBorder.AI
{
    public partial class SimpleAISystem : SystemBase
    {
            ...
            ref SimpleAIState aiState, AIDifficultyProfile profile, float now)
        {
            switch (g.Kind)
            {
                case GoalKind.Build:
                    ...
                    // the Hall claim uses.
                    //
                    // THE ESSENTIALS IGNORE THE RESERVATION TOO (2026-09-03).
                    // The claim reservation re-arms every attempt tick (12 s)
                    // for as long as any region is unclaimed, so "while
                    // saving" means "the whole match" -- and with only ~120
                    // supplies of working float spendable underneath it, the
                    // 220-supply first Barracks was refused "wallet short"
                    // from minute 0 to minute 22 in every logged match, era
                    // after era. Housing, the income huts (capped at 3 here;
                    // the node pipeline caps its own) and the FIRST Barracks
                ...
                case GoalKind.AgeUp:
                    ...
                    AIBudget.Reserve(brain.Owner, stage, now,
                        Cfg.ageUpSaveSeconds, Cfg.ageUpReservePriority);
                    return false;
            }
            return false;
        }
        ...
        {
            var q = QC_VeilstoneOutcroppingTagLocalTransform.Get(em, QT_VeilstoneOutcroppingTagLocalTransform);
