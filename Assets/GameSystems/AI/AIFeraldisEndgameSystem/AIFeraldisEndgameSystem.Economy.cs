...
namespace TheWaningBorder.AI
{
    public partial struct AIFeraldisEndgameSystem : ISystem
    {
        ...
        static readonly ComponentType[] QT_FeraldisWorkerTagFactionTagLocalTransform =
        {
            ComponentType.ReadOnly<FeraldisWorkerTag>(),
            ComponentType.ReadOnly<FactionTag>(),
            ComponentType.ReadOnly<LocalTransform>(),
        };
        static CachedEntityQuery QC_FeraldisWorkerTagFactionTagLocalTransform;
        ...
        static readonly ComponentType[] QT_WarTotemTagFactionTagLocalTransform =
        {
            ComponentType.ReadOnly<WarTotemTag>(),
            ComponentType.ReadOnly<FactionTag>(),
            ComponentType.ReadOnly<LocalTransform>(),
        };
        static CachedEntityQuery QC_WarTotemTagFactionTagLocalTransform;
        ...
        {
            var q = QC_FeraldisWorkerTagFactionTagLocalTransform.Get(em, QT_FeraldisWorkerTagFactionTagLocalTransform);
        ...
        {
            var q = QC_AIBrainAISharedKnowledge.Get(em, QT_AIBrainAISharedKnowledge);
            using var ents = q.ToEntityArray(Allocator.Temp);
            for (int i = 0; i < ents.Length; i++)
            ...
            float3 hallPos, out float3 spot)
        {
            ...
            var tq = QC_WarTotemTagFactionTagLocalTransform.Get(em, QT_WarTotemTagFactionTagLocalTransform);
            ...
            for (int y = 0; y < res; y++)
            {
                for (int x = 0; x < res; x++)
                {
                    ...
                    for (int e = 0; e < existing.Length && !crowded; e++)
                    {
                        ...
                        crowded = ex * ex + ez * ez < Cfg.minTotemSpacing * Cfg.minTotemSpacing;
                    }
                    if (crowded) continue;
