...
namespace TheWaningBorder.AI
{
    ...
    public partial class IntelSystem : SystemBase
    {
        ...
        static readonly ComponentType[] QT_FactionTagLocalTransformHealth =
        {
            ComponentType.ReadOnly<FactionTag>(),
            ComponentType.ReadOnly<LocalTransform>(),
            ComponentType.ReadOnly<Health>(),
        };
        static CachedEntityQuery QC_FactionTagLocalTransformHealth;
        ...
        /// they live in IntelSystem.asset now.</summary>
        static IntelSystemConfig Cfg => IntelSystemConfig.I;
        ...
        protected override void OnUpdate()
        {
            ...
            var worldQuery = QC_FactionTagLocalTransformHealth.Get(em, QT_FactionTagLocalTransformHealth);
                ...
                for (int j = 0; j < ents.Length; j++)
                {
                    ...
                    if (!Alliances.AreHostile(owner, facs[j].Value))
                    {
                        ...
                        if (hps[j].Value < hps[j].Max
                            && TransientState.Active<LastAttackerEntity>(em, ents[j])
                            && em.GetComponentData<LastAttackerEntity>(ents[j]).Value != Entity.Null)
                        {
                            ThreatMaps.Stamp(owner, pos, Cfg.damageThreatStamp);
                        }
                        continue;
                    }
