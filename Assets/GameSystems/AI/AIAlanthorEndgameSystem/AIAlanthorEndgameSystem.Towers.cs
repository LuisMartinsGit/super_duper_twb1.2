...
namespace TheWaningBorder.AI
{
    public partial struct AIAlanthorEndgameSystem : ISystem
    {
        ...
        static readonly ComponentType[] QT_WatchTowerTagFactionTagLocalTransform =
        {
            ComponentType.ReadOnly<WatchTowerTag>(),
            ComponentType.ReadOnly<FactionTag>(),
            ComponentType.ReadOnly<LocalTransform>(),
        };
        static CachedEntityQuery QC_WatchTowerTagFactionTagLocalTransform;

        static readonly ComponentType[] QT_GathererHutTagFactionTagLocalTransform =
        {
            ComponentType.ReadOnly<GathererHutTag>(),
            ComponentType.ReadOnly<FactionTag>(),
            ComponentType.ReadOnly<LocalTransform>(),
        };
        static CachedEntityQuery QC_GathererHutTagFactionTagLocalTransform;
            ...
            EntityManager em, Faction faction, float3 hallPos, out float3 nodePos)
        {
            ...
            float bestD2 = Cfg.unprotectedNodeRadius * Cfg.unprotectedNodeRadius;
            bool found = false;

            var veilQ = QC_VeilstoneOutcroppingTagLocalTransform.Get(em, QT_VeilstoneOutcroppingTagLocalTransform);
            var ironQ = QC_IronMineTagLocalTransform.Get(em, QT_IronMineTagLocalTransform);
        ...
        {
            var q = QC_WatchTowerTagFactionTagLocalTransform.Get(em, QT_WatchTowerTagFactionTagLocalTransform);
            ...
            float maxWalk, out float3 chokePos, out float3 chokeSide, out float chokeWidth)
        {
            chokePos = default;
            chokeSide = default;
            chokeWidth = Cfg.chokeWidthThreshold;
            bool choke = false;
            ...
            float3 hallPos, NativeList<float3> ownTowers, int2 towerSize, out float3 spot)
        {
            ...
            var q = QC_GathererHutTagFactionTagLocalTransform.Get(em, QT_GathererHutTagFactionTagLocalTransform);
            using var facs = q.ToComponentDataArray<FactionTag>(Allocator.Temp);
            using var xfs = q.ToComponentDataArray<LocalTransform>(Allocator.Temp);

            float covSq = Cfg.hutCoverageRadius * Cfg.hutCoverageRadius;
            float3 hut = default;
