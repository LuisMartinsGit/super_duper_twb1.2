// File: Assets/Scripts/Presentation/ChapelSiteDecals.cs
// BFME2-style build-plot decals — six ground markers in a hex ring around
// the player's completed Temple of Ridan. Clicking one opens the
// SectChoicePopup bound to the matching chapel slot index.
//
...
namespace TheWaningBorder.Presentation
{
    ...
    {
        public const float RingRadiusMeters = 7.0f;     // distance from temple centre
        public const float DecalSize        = 3.0f;     // edge length of the square decal
        ...
        void BuildDecals(Vector3 templeCentre)
        {
            ...
            {
                // Slot 0 sits at angle -90° (north of the temple) so the first
                // slot's decal is the one nearest to the camera's typical RTS
                // overhead-back view. Subsequent slots fan clockwise.
                float angle = -Mathf.PI * 0.5f + (Mathf.PI * 2f) * (i / (float)_decals.Length);
                float wx = templeCentre.x + Mathf.Cos(angle) * RingRadiusMeters;
                float wz = templeCentre.z + Mathf.Sin(angle) * RingRadiusMeters;
        ...
        void UpdateHover()
        {
            if (UnifiedUIManager.IsPointerOverAnyPanel() || SectChoicePopup.IsVisible)
            {
                SetHovered(-1);
                return;
            }
        ...
        void HandleClick()
        {
            ...
            if (UnifiedUIManager.IsPointerOverAnyPanel()) return;
            if (SectChoicePopup.IsVisible) return;
            ...
            SectChoicePopup.Show(tag.Temple, tag.SlotIndex, GameSettings.LocalPlayerFaction);
        }
