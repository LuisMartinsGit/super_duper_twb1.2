...
// Location: Assets/Scripts/Presentation/Buildings/BuildingEffectSystem.cs
...
namespace TheWaningBorder.Presentation
{
    ...
    public class BuildingEffectSystem : MonoBehaviour
    {
        ...
        private void UpdateCollapseAnimations(EntityManager em, float dt)
        {
            ...
            var query = _collapseQuery.Get(em, CollapseQueryTypes);
            ...
            for (int i = 0; i < entities.Length; i++)
            {
                ...
                // MESH renderers only (2026-08-04: same bug as construction dust
                // — a world-space particle child has near-map-sized bounds, and
                // encapsulating it centered a map-covering death cloud), and
                // clamped to a sane footprint regardless.
                Bounds buildingBounds = default;
                bool anyBounds = false;
                foreach (var r in go.GetComponentsInChildren<Renderer>())
                {
                    if (r is ParticleSystemRenderer || r is TrailRenderer || r is LineRenderer)
                        continue;
                    if (!anyBounds) { buildingBounds = r.bounds; anyBounds = true; }
                    else buildingBounds.Encapsulate(r.bounds);
                }
                if (!anyBounds)
                {
                ...
                float buildingRadius = Mathf.Min(10f,
                    Mathf.Max(buildingBounds.extents.x, buildingBounds.extents.z));
                float buildingHeight = Mathf.Min(20f, buildingBounds.size.y);
