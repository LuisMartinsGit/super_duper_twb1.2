// ProceduralCurseParticleGenerator.cs
// Builds a per-node ambient ParticleSystem for the Crystal-faction "cursed
// ground" — slow purple motes drifting from the node area. One particle
// system per crystal node (NOT per tile).
//
// Iteration 2 (2026-05-21, task-cursed-ground-luminous-crystals-111 item 6):
...
//
//   The polling is done by an attached CurseNodeAmbientDirector
//   MonoBehaviour that reads CrystalSpreadState via the EntityManager
...
//
// Location: Assets/Scripts/Presentation/ProceduralCurseParticleGenerator.cs
...
{
    /// <summary>
    /// Static factory for the per-crystal-node ambient particle drift.
    /// Returns a tracked GameObject the caller parents to the node's root.
    /// </summary>
    public static class ProceduralCurseParticleGenerator
    {
        // ---- Tunables ----
        ...
        public static GameObject Create(Vector3 nodePos, Entity nodeEntity, EntityManager em)
        {
            ...
            if (em != default && em.Exists(nodeEntity) && em.HasComponent<CrystalNode>(nodeEntity))
            {
                var cn = em.GetComponentData<CrystalNode>(nodeEntity);
                if (cn.SpreadRadius > 0.01f) spreadRadius = cn.SpreadRadius;
            }
            float emissionRadius = spreadRadius * EmissionRadiusRatio;

            var go = new GameObject($"CurseParticles_{nodeEntity.Index}");
            ...
            var director = go.AddComponent<CurseNodeAmbientDirector>();
        ...
        private static Material GetMoteMaterial()
        {
            ...
            var mat = new Material(shader) { name = "CurseMoteMat" };
        ...
        /// <summary>
        /// Reads the node's <see cref="CrystalSpreadState.CurrentRingRadius"/>
        ...
        /// full spread radius.
        /// </summary>
        public sealed class CurseNodeAmbientDirector : MonoBehaviour
        {
            ...
            private void Update()
            {
                if (_ps == null || !_emValid) return;
                _timer -= Time.deltaTime;
                if (_timer > 0f) return;
                _timer = PollIntervalSeconds;

                if (!_em.Exists(_node)) return;
                if (!_em.HasComponent<CrystalSpreadState>(_node)) return;
                var st = _em.GetComponentData<CrystalSpreadState>(_node);
