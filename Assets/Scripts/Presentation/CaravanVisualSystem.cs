...
namespace TheWaningBorder.Presentation
{
    public class CaravanVisualSystem : MonoBehaviour
    {
        ...
        private EntityQuery _caravanQuery;
        ...
        void Update()
        {
            if (_world == null || !_world.IsCreated)
            {
                ...
                _caravanQuery = _em.CreateEntityQuery(
                    ComponentType.ReadOnly<CaravanTag>(),
                    ComponentType.ReadOnly<LocalTransform>());
            }

            using var ents = _caravanQuery.ToEntityArray(Allocator.Temp);
            using var transforms = _caravanQuery.ToComponentDataArray<LocalTransform>(Allocator.Temp);
