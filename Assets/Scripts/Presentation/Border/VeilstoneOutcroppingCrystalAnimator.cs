// VeilstoneOutcroppingCrystalAnimator.cs
// Per-outcropping visual driver. Sits on the root of the instantiated
// P_VeilstoneOutcropping_Gem* GameObject. The cluster shows no animation during mining —
// it's "chipped away" implicitly as the patch loses nodes one by one. The
// only visual event is the full shatter when this node hits 0 veilstone.
//
// Death-handoff:
...
namespace TheWaningBorder.Presentation
{
    ...
    public sealed class VeilstoneOutcroppingCrystalAnimator : MonoBehaviour
    {
        [SerializeField] private VeilstoneOutcroppingOreNode _oreNode;

        private bool _dying;

        private void Awake()
        {
            if (_oreNode == null) _oreNode = GetComponentInChildren<VeilstoneOutcroppingOreNode>();
        }
        ...
        public void BeginDeath()
        {
            ...
            // Fog-hidden views are deactivated; StartCoroutine on an inactive
            // GameObject throws and the view would leak. A shatter no one can
            // see (and whose 2D audio would leak intel) is skipped entirely.
