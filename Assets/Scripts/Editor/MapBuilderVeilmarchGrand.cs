...
//   * rim flanks used to be a fixed off-axis vector rotated by the sector
//     angle, which at 45 degrees lands at y = 530 -- off the map. They are
//     now placed by polar angle at a radius inside the inscribed circle.
//   * far corners: a square has FOUR, whatever the sector count.
//   * the angular offsets of the ridge blobs, gate woods and home wood are
//     halved, or a 45-degree sector's blobs would run into its neighbour's.
// Everything else is the original generator, verbatim.
...
namespace TheWaningBorder.Core.Maps.EditorTools
{
    public static class MapBuilderVeilmarchGrand
    {
        ...
        private const float RimR = 470f;          // inside the 512 m inscribed circle
        private const float RimOffDeg = 14f;

        // The ridge ring and its gates.
        private const float RidgeRingR = 280f;
        private const float RidgeBlobSize = 55f;
        private const float RidgeBlobOffDeg = 12.5f;   // blobs at cardinal +/-25 deg

        // Forest stands: gate-narrowing pairs, one wood per home, one per corner.
        private const float GateForestR = 300f;
        private const float GateForestOffDeg = 6.5f;
        ...
        private static readonly string[] SectorNames =
            { "Eastmarch", "Northeastmarch", "Northmarch", "Northwestmarch",
              "Westmarch", "Southwestmarch", "Southmarch", "Southeastmarch" };

        /// <summary>Diagonal names, counter-clockwise from north-east.</summary>
        private static readonly string[] DiagNames =
            { "East-Northeast", "North-Northeast", "North-Northwest", "West-Northwest",
              "West-Southwest", "South-Southwest", "South-Southeast", "East-Southeast" };
        /// <summary>The square's four corners, counter-clockwise from north-east.</summary>
        ...
        private static void PlaceMarkers()
        {
            ...
            // 01-08 — the Veilfields: the ONLY veilstone/veilsteel on the map.
            for (int i = 0; i < Sectors; i++)
                NewSeed(regionRoot, ref idx, Polar(CentreRingR, DiagAngle(i)),
                        $"{DiagNames[i]} Veilfield");

            // 09-16 — the home territories (large).
            for (int i = 0; i < Sectors; i++)
                NewSeed(regionRoot, ref idx, Polar(HomeR, CardinalAngle(i)),
                        $"{SectorNames[i]} Home");

            // 17-24 — corner fields (small).
            for (int i = 0; i < Sectors; i++)
                NewSeed(regionRoot, ref idx, Polar(CornerR, DiagAngle(i)),
                        $"{DiagNames[i]} Corner");

            // 25-40 — rim flanks (small), two hugging each home along the map
            // edge so the home region cannot balloon down the rim.
            // NO RIM FLANKS AT EIGHT SECTORS (2026-09-13). They exist on the
            // four-player map to stop a home region ballooning down the rim.
            // With eight homes on the same ring the rim is already crowded,
            // and keeping sixteen flank seeds cut the homes down to 2.1 ha
            // against 1.6 ha smalls -- a ratio of 1.27 where the design wants
            // 3 small = 1 large. Without them the rim ground returns to the
            // homes. The rim RESOURCES below are kept and seated by polar
            // angle, so each home still carries its edge iron and supply.
            // (Sixteen seeds not created, and their resources with them.)

            // 41-48 — diagonal fields between the Veilfields and the corners.
            // 49-52 — far corners, the map-corner wedges.
            //
            ...
            for (int i = 0; i < 4; i++)
                NewSeed(regionRoot, ref idx,
                        Rotate(new Vector2(FarCornerXY, FarCornerXY), i * Mathf.PI * 0.5f),
                        $"{CornerNames[i]} Far Corner");

            // DIAGONAL RIM SEEDS (2026-09-13). The area audit wants homes at
            // 2.3-4.2x a small field, and the four-player map gets there by
            // splitting the smalls. At eight sectors every wedge is half as
            // wide, so the homes cannot grow; the smalls have to shrink. One
            // more seed per diagonal, on the rim behind the corner field,
            // splits each corner/far region without touching a home.
            for (int i = 0; i < Sectors; i++)
                NewSeed(regionRoot, ref idx, Polar(498f, DiagAngle(i)),
                        $"{DiagNames[i]} Rim");
            ...
            for (int i = 0; i < Sectors; i++)
            {
                ...
                Forest(natureRoot, $"Home Wood {SectorNames[i]}",
                       Polar(398f, a + 8f * Mathf.Deg2Rad), HomeForestSize);
            ...
            for (int i = 0; i < Sectors; i++)
            {
                ...
                // Far corners are placed once, after this loop: four, not Sectors.

                // Diagonal rim (the seed at 498 m behind the corner field):
                // iron + two supply, the same ration the four-player map's
                // rim flanks carry. SeatIn keeps every node inside the region
                // the seed actually won, so nothing lands in a home (the home
                // already holds its quota of exactly four supply nodes).
                var rp = Polar(498f, d);
                int rim = TheWaningBorder.World.Regions.RegionMap.RegionAt(rp.x, rp.y);
                Iron(resRoot, $"Iron {DiagNames[i]} Rim", SeatIn(rim, 492f, d), 24);
                Supply(resRoot, $"Supply {DiagNames[i]} Rim a", SeatIn(rim, 486f, d - 0.05f));
                Supply(resRoot, $"Supply {DiagNames[i]} Rim b", SeatIn(rim, 500f, d + 0.05f));
            }

            // The four far corners: iron + supply each.
            for (int c = 0; c < 4; c++)
            {
                var fc = Rotate(new Vector2(FarCornerXY, FarCornerXY), c * Mathf.PI * 0.5f);
                int far = TheWaningBorder.World.Regions.RegionMap.RegionAt(fc.x, fc.y);
                float fa = Mathf.Atan2(fc.y, fc.x);
                float fr = fc.magnitude;
                Iron(resRoot, $"Iron {CornerNames[c]} Far", SeatIn(far, fr - 16f, fa), 24);
                Supply(resRoot, $"Supply {CornerNames[c]} Far a", SeatIn(far, fr - 30f, fa + 0.04f));
                Supply(resRoot, $"Supply {CornerNames[c]} Far b", SeatIn(far, fr + 8f, fa - 0.03f));
            }
        ...
        private static int ValidateRegionAreas()
        {
            ...
            // Per-region breakdown, largest first, so a failing ratio says
            // WHICH small regions to split instead of only the average.
            var rows = new List<KeyValuePair<int, float>>(area);
            rows.Sort((x, y) => y.Value.CompareTo(x.Value));
            var sb = new System.Text.StringBuilder();
            foreach (var kv in rows)
            {
                if (kv.Key == 0) continue;
                bool isHome = kv.Key >= 1 + Sectors && kv.Key < 1 + Sectors * 2;
                sb.Append(isHome ? " H" : " ").Append(kv.Key).Append('=')
                  .Append((kv.Value / 10000f).ToString("0.0")).Append("ha");
            }
            Debug.Log($"[{MapName}] AREA BY REGION (ha, H = home):{sb}");
