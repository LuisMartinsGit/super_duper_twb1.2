        ...
        private void OnTechCompleted(Faction faction, string techId)
        {
            ...
            switch (techId)
            {
                ...
                case "MasonGuild":
                    ...
                    break;
                // The two BOUGHT wall levels, researched at the Wall Hub
                // (docs/Design/Age_1_Alanthor.md § The four wall levels).
                // Each re-clads EVERYTHING the faction owns, so a wall is
                // never a patchwork of levels. Lv1 (stone) has no tech —
                // AgeUpSystem grants it with the Alanthor culture pick.
                case "Battlements":
                    TheWaningBorder.Entities.AlanthorWall.PromoteFactionWalls(
                        em, faction, TheWaningBorder.Entities.WallTiers.Battlemented);
                    break;
                case "ShieldedRamparts":
                    TheWaningBorder.Entities.AlanthorWall.PromoteFactionWalls(
                        em, faction, TheWaningBorder.Entities.WallTiers.Shielded);
                    break;
