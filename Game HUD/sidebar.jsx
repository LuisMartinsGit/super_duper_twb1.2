// sidebar.jsx — Sects rail. Six sects stacked inside one jade-and-silver
// frame (matched to the resource panel's vocabulary). Each row is the sect
// name on the left and three small hex buttons on the right:
//   [Level-Up]  [Passive (hover only)]  [Active cast]
// A handle on the right edge slides the whole panel off to the left.
...
function HexButton({ theme, variant, sect, onAction }) {
  ...
  const cfg = variant === 'active' ? {
    icon: sect.active.icon,
    title: sect.active.label,
    kicker: 'Active',
    hint: sect.active.hint,
    glow: theme.accent,
    muted: false,
  } : variant === 'passive' ? {
    icon: 'eye',
    title: sect.passive.label,
    kicker: 'Passive',
    hint: sect.passive.hint,
    glow: theme.inlay,
    muted: true,
  } : {
    icon: 'levelup',
    ...
    glow: theme.accent,
    muted: sect.level >= sect.maxLevel,
  };
    ...
    >
      <svg width="28" height="25" viewBox="0 0 56 49" className="hex-btn-svg">
        ...
        <polygon points={HEX_POINTS} fill={`url(#${id}-fill)`}
                 stroke={`url(#${id}-bezel)`} strokeWidth="1.6"
                 strokeLinejoin="round" />
        {showOrn && (
          <polygon points={HEX_INNER_POINTS} fill="none"
                   stroke={theme.inlayShadow} strokeWidth="0.6"
        ...
        <polygon className="hex-btn-glow" points={HEX_POINTS}
                 fill="none" stroke={cfg.glow} strokeWidth="1.5"
...
function Sidebar({ theme, onItem }) {
  ...
  return (
    <div className={`sect-host ${collapsed ? 'collapsed' : ''}`}>
      {collapsed ? (
        <PowersRail theme={theme} onItem={onItem} />
      ) : (
        <SectsRail theme={theme} onItem={onItem} />
      )}

      {/* Collapse / expand handle — sits to the right of whichever rail is showing. */}
      <button
...
// Full sects/religion rail — sect name + Level / Passive / Active triplet per row.
function SectsRail({ theme, onItem }) {
  const lvl = theme.ornament;
  return (
        ...
        <div className="sect-stack">
          {SECTS.map((s, i) => (
            <React.Fragment key={s.key}>
              ...
              <SectRow sect={s} theme={theme} onAction={(k, v) => onItem && onItem(k, v)} />
            </React.Fragment>
          ))}
        </div>
      </div>
...
// Compact Powers rail — only the 6 active sect abilities, stacked vertically.
// Renders WHEN the sects rail is collapsed. Same chrome vocabulary, narrower.
function PowersRail({ theme, onItem }) {
  const lvl = theme.ornament;
  return (
    <div className="rc-root rc-v powers-rail" style={{
      ...
      <div className="rc-frame powers-frame">
        ...
        <div className="rc-inlay" />
        <div className="rc-corner rc-corner-tl">
          <FiligreeCorner size={28} color={theme.inlay} dim={theme.inlayDim} accent={theme.accent} level={lvl} />
        </div>
        <div className="rc-corner rc-corner-tr">
          <FiligreeCorner size={28} color={theme.inlay} dim={theme.inlayDim} accent={theme.accent} level={lvl} />
        </div>
          ...
          {SECTS.map((s, i) => (
            <React.Fragment key={s.key}>
              ...
              <div className="powers-row">
                <HexButton theme={theme} variant="active" sect={s} onAction={(k, v) => onItem && onItem(k, v)} />
              </div>
            </React.Fragment>
          ))}
        </div>
...
Object.assign(window, { Sidebar, HexButton, SectRow });
