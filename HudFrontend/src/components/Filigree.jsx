// Reusable ornate SVG flourishes for HUD frames. Pure geometry: arcs, lines
// and dots arranged symmetrically. Strokes scale with ornament level.

export function FiligreeCorner({ size = 60, color = '#cfd6d3', accent = '#e8b84a', dim = '#7f8e8a', level = 'maximal' }) {
  const W = size;
...
export function FiligreeEdge({ width = 240, height = 24, color = '#cfd6d3', accent = '#e8b84a', dim = '#7f8e8a', level = 'maximal' }) {
...
export function FiligreeMedallion({ size = 60, color, accent, dim, gem = '#1d6a55', gemHi = '#3fbf9a', level = 'maximal', label }) {
  const sw = level === 'minimal' ? 1.0 : level === 'balanced' ? 1.2 : 1.4;
  const idBase = `med-${size}-${(color || '#fff').slice(1)}`;
  return (
    <svg width={size} height={size} viewBox="0 0 60 60" style={{ display: 'block', pointerEvents: 'none' }}>
      <defs>
        <radialGradient id={`${idBase}-gem`} cx="50%" cy="40%" r="60%">
      ...
      <g fill="none" stroke={color} strokeWidth={sw} strokeLinecap="round">
        ...
        <path d="M 30 12 L 42 30 L 30 48 L 18 30 Z" fill={`url(#${idBase}-gem)`} />
...
export function IconDisc({ size = 44, color, dim, accent, level = 'maximal', children }) {
  const sw = level === 'minimal' ? 1.0 : level === 'balanced' ? 1.2 : 1.4;
  const R = size / 2;
  const gradId = `disc-${size}-${(color || '#fff').slice(1)}`;
  return (
